...
export default function SettingsPage() {
  ...
  const { settings, updateSettings, mounted } = useUserStore();
  ...
  return (
    <main className="flex-1 pt-24 pb-16 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl space-y-8">
        ...
        {/* 3. About info */}
        <section className="rounded-2xl bg-zinc-50 dark:bg-zinc-900/60 p-6 border border-zinc-200/60 dark:border-zinc-800/60 text-xs text-zinc-500 space-y-2">
          <div className="flex items-center justify-between">
            <span className="font-semibold text-zinc-800 dark:text-zinc-200">
              Phiên bản ứng dụng
            </span>
            <span className="font-mono text-zinc-400">Cinépvq v1.0.0 (Production Ready)</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="font-semibold text-zinc-800 dark:text-zinc-200">
              Cơ chế lưu trữ cục bộ
            </span>
            <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-medium">
              <CheckCircle2 className="h-3.5 w-3.5" />
              Đã kích hoạt & sẵn sàng tích hợp backend
            </span>
          </div>
        </section>
      </div>
