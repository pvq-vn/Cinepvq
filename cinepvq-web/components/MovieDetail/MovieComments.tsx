...
export default function MovieComments({ movieSlug }: MovieCommentsProps) {
  ...
  // Load comments from API / Database when available
  useEffect(() => {
    if (!movieSlug) return;
    let isCancelled = false;

    fetch(`/api/comments?slug=${encodeURIComponent(movieSlug)}`)
      .then((res) => res.json())
      .then((data) => {
        if (!isCancelled && data.status === "success" && Array.isArray(data.comments) && data.comments.length > 0) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Asynchronously save to PostgreSQL if authenticated
    if (user?.id && movieSlug) {
      fetch("/api/comments", {
        method: "POST",
